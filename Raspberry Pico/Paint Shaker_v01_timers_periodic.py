...
#i2c = I2C(scl=Pin(19),sda=Pin(18),freq=400000)     # I2C1
...
def run_HCSRO4(a):
    global thread_please_exit, xva, yva, zva, xv, yv, zv 
    x = HCSR04(trigger_pin=10, echo_pin=9)
    y = HCSR04(trigger_pin=12, echo_pin=11)
    z = HCSR04(trigger_pin=14, echo_pin=13) 
    while True:
      ...
      if thread_please_exit:
        print("Exiting Thread")
        _thread.exit()
      
# start this on core1
_thread.start_new_thread(run_HCSRO4,(1, ))
# ----- end core1 ------------------------
...
if TIMERS_ACTIVE:
  ...
  #UITimer=Timer()
  #UITimer.init(mode=Timer.PERIODIC, freq=2100, callback=UICheckTick) # Sweep through all the UI buttons and pots. 
  ...
  #CommsTimer.init(mode=Timer.PERIODIC, freq=2150, callback=CommsSendTick)
  
  DebugTimer=Timer()
  DebugTimer.init(mode=Timer.PERIODIC, freq=2000, callback=DebugTick)    # 1 second count of main loop.
