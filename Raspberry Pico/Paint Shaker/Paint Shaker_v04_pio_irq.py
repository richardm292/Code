...
# globals x,y,z latest reading and averaged.
xv=0
xva=0
yv=0
yva=0
...
def UISweep():
    ...
    ctl[SPARE_BUTTON1] = checkbutton(ctl[SPARE_BUTTON1], (spare_button1.value()), "S1")
    ctl[SPARE_BUTTON2] = checkbutton(ctl[SPARE_BUTTON2], (spare_button2.value()), "S2")
...
def HCSR04Collect(index):
    global xva, yva, zva, xv, yv, zv, x, y, z
    if index == 0:
...
COMMS_INTERVAL = const(10)
UI_INTERVAL = const(20)
DEBUG_INTERVAL = const(200)
...
@micropython.native
def timer_tick_inc(q):
    global timer_tick, timer_flags
    timer_tick = ( timer_tick + 1 ) % 50000
    flag = timer_flags
    flag = flag | ((((timer_tick % UI_INTERVAL) == 0) <<7) & (1<<7))
    flag = flag | ((((timer_tick % COMMS_INTERVAL) == 0) <<6) & (1<<6))
    flag = flag | ((((timer_tick % DEBUG_INTERVAL) == 0) <<5) & (1<<5))
    timer_flags = flag
    
@rp2.asm_pio()
def irq_freq_100():     # expecting freq=2000
    nop()          [18] # 
    irq(0)              # 20 cycles

sm = rp2.StateMachine(0, irq_freq_100, freq=2000)
rp2.PIO(0).irq(timer_tick_inc)
sm.active(1)
