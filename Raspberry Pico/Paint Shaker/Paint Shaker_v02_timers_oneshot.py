...
# GPIO16
# GPIO17
i2c = I2C(id=1,scl=Pin(19),sda=Pin(18),freq=100000)     # I2C1
...
if TIMERS_ACTIVE:
  ...
  DebugTimer=Timer()
  DebugTimer.init(mode=Timer.ONE_SHOT, period=1000, callback=DebugTick)    # 1 second count of main loop.
...
while ctl[BREAK_BUTTON]:
  ...
  if UISweepFlag:
    UISweep()
    UISweepFlag = False
    UITimer.init(mode=Timer.ONE_SHOT, period=75, callback=UICheckTick)     # 10 times / second
  if CommsSendFlag:
    CommsSend()
    CommsSendFlag = False
  ...
  if DebugFlag:
    Debug()
    DebugFlag = False
    DebugTimer.init(mode=Timer.ONE_SHOT, period=1000, callback=DebugTick)  # 5 seconds
  if HCSO4CollectFlag:
